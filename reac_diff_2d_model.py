...
def get_model(num_visible, num_hidden, num_der, mesh, dx, dt, scale, get_dzdt=False):
    ...
    def sym_model(z, t):
        return SymModel(
            dt,
            (
                SpatialDerivative2D(mesh, dx, init=jnp.zeros),
    ...
    if multi_gpu:
        ...
        for i in range(n_devices):
            ...
            if loss_fn_args["reg_dzdt"] is not None:
                ...
                batch.append(scaled_data[None, start:end, :, :, :])
...
if __name__ == "__main__":
    ...
    dt = 5e-2
    tspan = (0, 50 + 4 * dt)
    scaled_data, scale = get_dataset(
        args.dataset, generate_dataset, sys_size=sys_size, mesh=mesh, dt=dt, tspan=tspan
    )
    ...
    params["encoder"] = model_init["encoder"](
        next(key_seq), jnp.ones([1, scaled_data.shape[1], mesh, mesh, num_visible])
    )
