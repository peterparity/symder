...
def generate_dataset(dt=1e-2, tmax=None, raw_sol=False):
    if tmax is None:
        tmax = 100 + 4*dt
    ...
    sol = solve_ivp(lorenz, (0, tmax), y0=np.stack((u0, v0, w0), axis=-1), t_eval=t_eval, args=(sigma, beta, rho))
    data = sol.y[[0,1]]
    ...
    scaled_data = scaled_data.reshape(scaled_data.shape[0], data.shape[0], data.shape[1])

    if raw_sol:
        return scaled_data, scaler.scale_.reshape(2, num_der+1), sol.y

    return scaled_data, scaler.scale_.reshape(2, num_der+1)